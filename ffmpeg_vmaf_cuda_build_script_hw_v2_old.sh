...
export LIBRARY_PATH="/usr/local/lib:/usr/local/cuda-13.0/lib64:/usr/local/cuda-13.0/targets/x86_64-linux/lib"
...
DAV1D_TAG="master" # Using 'master' branch for DAv1D
...
cd "$HOME/build_temp/"
if [ -f "$HOME/build_temp/dav1d/build/src/libdav1d.a" ]; then
    echo "Verification: libdav1d.a found at $HOME/build_temp/dav1d/build/src/"
else
    echo "ERROR: libdav1d.a NOT found at $HOME/build_temp/dav1d/build/src/. VMAF static library build might have failed."
    exit 1
fi
...
DAV1D_BUILD_LIB_DIR="${HOME}/build_temp/dav1d/build/src"
DAV1D_SRC_INCLUDE_DIR="${HOME}/build_temp/dav1d/build/src"
DAV1D_PUBLIC_INCLUDE_DIR="${HOME}/build_temp/dav1d/build/include"
...
./configure \
  ...
  --ld="g++" \
  --extra-cflags="-I/usr/local/include -I${CUDA_PATH}/include -I${VMAF_SRC_INCLUDE_DIR} -I${VMAF_PUBLIC_INCLUDE_DIR} -I${DAV1D_SRC_INCLUDE_DIR} -I${DAV1D_PUBLIC_INCLUDE_DIR}" \
  --extra-ldflags="-L/usr/local/lib -L${CUDA_PATH}/lib64 -L${VMAF_BUILD_LIB_DIR} -L${DAV1D_BUILD_LIB_DIR}" \
