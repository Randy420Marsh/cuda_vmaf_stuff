...
#    from NVIDIA's website for Ubuntu 24.04/22.04.
...
#----------------------------------#
#Fix nvcudid library not found at runtime
#I needed to do this i guess my system is bit of a mess right now

#echo "/usr/lib/x86_64-linux-gnu" | sudo tee /etc/ld.so.conf.d/00-nvidia-fix.conf

# Verify the file actually exists first (it should return a file path)

#ls /usr/lib/x86_64-linux-gnu/libnvcuvid.so*
...
# --- 0. PRE-FLIGHT CLEANUP (CRITICAL FIX) ---
# Remove old FFmpeg libraries that might confuse the linker
echo "Cleaning up old/zombie FFmpeg libraries to prevent linker conflicts..."
sudo rm -f /usr/local/lib/libavcodec.*
sudo rm -f /usr/local/lib/libavformat.*
sudo rm -f /usr/local/lib/libavutil.*
sudo rm -f /usr/local/lib/libavdevice.*
sudo rm -f /usr/local/lib/libavfilter.*
sudo rm -f /usr/local/lib/libswscale.*
sudo rm -f /usr/local/lib/libswresample.*
sudo rm -f /usr/local/lib/libpostproc.*
sudo ldconfig
...
cd "$HOME/build_temp"
rm -rf lcms2-${LCMS_VERSION} # Clean up source
...
# FIX APPLIED: Added -Ddemos=disabled to prevent 'plplay' from trying to link against
# a non-existent or broken libavcodec before FFmpeg is built.
meson setup build \
    --prefix="$HOME/build_temp" \
    --default-library=static \
    --libdir=lib \
